 ...
 */
public class ChargeStationXRocketMQConsumer {

    /**
     * 日志标签，用于标识日志信息来源
     */
    private final static String TAG = "充电桩RocketMQ消费者";
    ...
    public static void init() {
        ...
        for (Method method : ChargeStationXRocketMQConsumer.class.getDeclaredMethods()) {
        ...
        // 设置VIP通道
        X_ROCKET_MQ.getConsumerClient().setVipChannelEnabled(VipChannelEnabled);
        // 设置线程最大使用量
...
//                    , context.getMessageQueue().getTopic());
    ...
    private static SyncResult OfflineMonitorTask(JSONObject message) {
...
//            return new SyncResult(0, "");
        }, String.format("%s:OfflineMonitor:%s", GROUP_NAME, CSId), ECacheTime.HOUR * 8, null);
        return new SyncResult(0, "");
    }
    ...
    private static SyncResult AbnormalTask(JSONObject message) {
        String CSId = JsonUtil.getString(message, "CSId");
        String name = JsonUtil.getString(message, "name");
        // 减少通知频率
    ...
    private static SyncResult DaySummaryTaskV2(JSONObject message) {
        String CSId = JsonUtil.getString(message, "CSId");
        String name = JsonUtil.getString(message, "name");
...
//        ChargeStationSummaryTaskJobV2.execute(CSId);
    ...
    private static SyncResult MonthSummaryTaskV2(JSONObject message) {
        String CSId = JsonUtil.getString(message, "CSId");
        String name = JsonUtil.getString(message, "name");
        long start_time = JsonUtil.getLong(message, "start_time");
        long end_time = JsonUtil.getLong(message, "end_time");
    ...
    private static SyncResult SummaryTaskV2(JSONObject message) {
        String CSId = JsonUtil.getString(message, "CSId");
        String name = JsonUtil.getString(message, "name");
