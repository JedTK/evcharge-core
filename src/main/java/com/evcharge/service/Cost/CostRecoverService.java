...
import com.evcharge.entity.cost.CostRecoverItemEntity;
 ...
 * 项目成本回收
 */
public class CostRecoverService {
    /**
     * 标签
     */
    ...
    // 单例模式的静态实例
    private volatile static CostRecoverService _this;
     ...
     * @return
     */
    public static CostRecoverService getInstance() {
        if (_this == null) {
            synchronized (CostRecoverService.class) {
                if (_this == null) _this = new CostRecoverService();
            }
        }
        return _this;
    }
     ...
     * @return
     */
    public ISyncResult create(@NonNull String project_code, @NonNull String cs_id) {
        if (StringUtil.isEmpty(project_code) && StringUtil.isEmpty(cs_id)) {
            return new SyncResult(2, "请选择项目或站点");
        }

        // 查询可回收的物料数据
        List<CostItemEntity> costItemEntityList = CostItemEntity.getInstance()
                .whereOr("project_code", project_code)
                .whereOr("cs_id", cs_id)
                .where("is_recyclable", 1)
                .selectList();
        if (costItemEntityList == null || costItemEntityList.isEmpty()) return new SyncResult(2, "无可回收的物料");

        return CostRecoverItemEntity.getInstance().beginTransaction(connection -> {
            for (CostItemEntity item : costItemEntityList) {
                CostRecoverItemEntity recoverItemEntity = new CostRecoverItemEntity();
                recoverItemEntity.project_code = project_code;
                recoverItemEntity.cs_id = cs_id;
                recoverItemEntity.item_name = item.item_name;
                recoverItemEntity.category_code = item.category_code;
                recoverItemEntity.spu_code = item.spu_code;
                recoverItemEntity.spec = item.spec;
                recoverItemEntity.unit = item.unit;
                recoverItemEntity.unit_price = item.unit_price;
                recoverItemEntity.quantity = item.quantity;
                recoverItemEntity.total_amount = item.total_amount;
                recoverItemEntity.purchase_channel = item.purchase_channel;
                recoverItemEntity.invoice_type = item.invoice_type;
                recoverItemEntity.invoice_tax_rate = item.invoice_tax_rate;
                recoverItemEntity.remark = item.remark;
                recoverItemEntity.create_time = TimeUtil.getTimestamp();
                recoverItemEntity.update_time = recoverItemEntity.create_time;
                if (recoverItemEntity.insertTransaction(connection) == 0) {
                    return new SyncResult(4, "无法添加数据");
                }
            }
    ...
    public ISyncResult getList(@NonNull String project_code
        ...
        String countCacheKey = String.format("Cost:RecoverItems:%s%s:TotalCount", project_code, cs_id);
        String listCacheKey = String.format("Cost:RecoverItems:%s%s:%s_%s", project_code, cs_id, limit, page);
        ...
        try {
            ...
            count = CostRecoverItemEntity.getInstance()
                    .whereOr("project_code", project_code)
                    .whereOr("cs_id", cs_id)
                    .count();
            if (count == 0) return new SyncResult(1, "");

            List<Map<String, Object>> list = CostRecoverItemEntity.getInstance()
                    .field("id,project_code,cs_id,item_name,category_code,category_name"
                            + ",spu_code,spec,unit,unit_price,quantity,total_amount,purchase_channel"
                            + ",invoice_type,invoice_rate,remark,admin_id,update_time"
                    )
                    .alias("c")
    ...
    public ISyncResult batchPut(@NonNull String project_code
            , @NonNull String cs_id
            , @NonNull List<CostRecoverItemEntity> item_list
            , long admin_id
    ) {
        if (StringUtil.isEmpty(project_code) && StringUtil.isEmpty(cs_id)) {
            return new SyncResult(2, "请选择项目或站点");
        ...
        SyncResult r = CostProjectEntity.getInstance().beginTransaction(connection -> {
            for (CostRecoverItemEntity item : item_list) {
                // 计算总金额
