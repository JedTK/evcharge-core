...
/**
 * 用户汇总平台数据 - 针对单个用户的数据汇总;
 *
 * @author : JED
 * @date : 2024-1-20
 */
public class UserPlatformSummaryEntity extends BaseEntity implements Serializable {
    //region -- 实体类属性 --
    /**
     * id
     */
     ...
     * 计次充电的次数
     */
    public Integer pay_per_charge_count;
    /**
     * 计次充电的时长
     */
     ...
     * 充电卡充电的次数
     */
    public Integer card_charge_count;
    /**
     * 充电卡充电的时长
     */
     ...
     * 充值订单数
     */
    public Integer rechargeOrderCount;
    /**
     * 充值金额（毛）
     */
     ...
     * 充值退款订单数
     */
    public Integer rechargeRefundOrderCount;
    /**
     * 充值退款订单金额
     */
     ...
     * 充电卡订单数
     */
    public Integer chargeCardOrderCount;
    /**
     * 充电卡金额（毛）
     */
     ...
     * 充电卡退款订单数
     */
    public Integer chargeCardRefundOrderCount;
    /**
     * 充电卡退款订单金额
     */
     ...
     * 历史最高功率
     */
    public BigDecimal historyMaxPower;
    /**
     * 注册时间
     */
    public long regTime;
     ...
     * @return
     */
    public static UserPlatformSummaryEntity getInstance() {
        return new UserPlatformSummaryEntity();
    }
     ...
     * @return
     */
    public SyncResult syncData(long uid) {
        LogsUtil.info("用户汇总平台数据", "[%s] 汇总数据任务执行", uid);
        try {
            ...
            if (getInstance().where("uid", uid).exist()) {
                getInstance().where("uid", uid).update(data);
            } else {
                data.put("create_time", TimeUtil.getTimestamp());
                getInstance().insert(data);
            }
        } catch (Exception e) {
            LogsUtil.error(e, "用户汇总平台数据", "汇总数据发生错误，uid=%s", uid);
        }
     ...
     */
    public void repairData(long start_uid) {
        ...
        syncData(uid);
        ThreadUtil.getInstance().execute("", () -> repairData(uid));
    }
}
