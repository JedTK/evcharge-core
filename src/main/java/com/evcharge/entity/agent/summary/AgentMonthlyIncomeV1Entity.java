...
public class AgentMonthlyIncomeV1Entity extends BaseEntity implements Serializable{
    //region -- 实体类属性 --
    /**
     * id
     */
     ...
     * @return
     */
    public static AgentMonthlyIncomeV1Entity getInstance() {
        return new AgentMonthlyIncomeV1Entity();
    }

    private final static String TAG = "元气充代理-消费数据-月汇总v1";
...
//
            ...
            for (AgentToOrganizeEntity nd : list) {
                if (useRocketMQ) {
                    ...
                    XRocketMQ.getGlobal().pushOneway(AgentXRocketMQConsumerV2.TOPIC, "AgentMonthSummaryTaskV1", rocketMQData);
                } else {
                    // 普通线程任务
    ...
    public SyncResult syncTaskJob(String organizeCode, long dateTimestamp) {
        ...
        try {
            ...
            Map<String, Object> chargeConsumeData = AgentStationMonthlyIncomeV1Entity.getInstance()
                    .field("IFNULL(SUM(charge_consume),0) AS charge_consume")
                    .where("organize_code",organizeCode)
                    .where("month_time", ">=", startTime)
                    .where("month_time", "<=", endTime)
                    .find();
            ...
            Map<String, Object> cardChargeData = AgentStationMonthlyIncomeV1Entity.getInstance()
            ...
            Map<String, Object> totalConsumeData = AgentStationMonthlyIncomeV1Entity.getInstance()
            ...
            Map<String, Object> splitAmountData = AgentStationMonthlyIncomeV1Entity.getInstance()
            ...
            if (getInstance().where("organize_code", organizeCode).where("month_time", startTime).exist()) {
                getInstance().where("organize_code", organizeCode).where("month_time", startTime).update(data);
            } else {
