...
public class AgentStationDailyIncomeV1Entity extends BaseEntity implements Serializable {
    //region -- 实体类属性 --
    /**
     * id
     */
     ...
     * @return
     */
    public static AgentStationDailyIncomeV1Entity getInstance() {
        return new AgentStationDailyIncomeV1Entity();
    }


    private final static String TAG = "元气充代理-充电桩数据-日汇总v1";
    ...
    public void startSyncTask(long starTime, long endTime, boolean useRocketMQ) {
        ...
        // 标准化时间戳
        starTime = TimeUtil.toTimestamp00(starTime);
        endTime = TimeUtil.toTimestamp00(endTime);
...
//                long taskStartTime = Math.max(online_time, starTime);
                if (useRocketMQ) {
                    ...
                    XRocketMQ.getGlobal().pushOneway(AgentXRocketMQConsumerV2.TOPIC, "DaySummaryTaskV1", rocketMQData);
                } else {
                    // 普通线程任务
                    ...
                    ThreadUtil.getInstance().execute(
                            String.format("[%s-%s] %s", organizeCode, CSId, TAG),
                            () -> syncData(organizeCode, CSId, finalStarTime, finalEndTime)
                    );
                }
            }
        }
