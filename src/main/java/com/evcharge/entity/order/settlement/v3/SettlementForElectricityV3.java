 ...
 */
@Deprecated
public class SettlementForElectricityV3 implements IChargingSettlementV3 {
    ...
    private SyncResult chargeFinishWithBalance(ChargeOrderV3Entity orderEntity
        ...
        return orderEntity.beginTransaction(connection -> {
            try {
                ...
                //电费金额 = 电量 * 价格
                BigDecimal electricityFeeAmount = billingElecAmountTEMP(orderEntity.totalElectricity, orderEntity.chargeStandardConfigId);
                //服务费金额 = 电量 * 价格
                BigDecimal serviceFeeAmount = billingServiceAmountTEMP(orderEntity.totalElectricity, orderEntity.chargeStandardConfigId);
                //安全充电保险费用
                BigDecimal safeChargeFee = new BigDecimal(orderEntity.safeChargeFee)
    ...
    /**
     * 计算电费 = 电量 * 价格
     * TODO 分时段计算电费（这里只是简单计算，针对单一时段计费而已），计费需要考虑分时段计费，没那么简单，这里暂时使用固定公式，后面需要进行优化
     *
     * @return
     */
    public static BigDecimal billingElecAmountTEMP(BigDecimal totalElectricity, long chargeStandardConfigId) {
        if (totalElectricity.compareTo(BigDecimal.ZERO) == 0) {
            LogsUtil.warn(TAG, "计算电费错误，无总电量");
            return new BigDecimal(0);
        }
        if (chargeStandardConfigId == 0) {
            LogsUtil.warn(TAG, "计算电费错误，计费配置值不能为0");
            return new BigDecimal(0);
        }

        List<ChargeStandardEnergyItemEntity> itemList = ChargeStandardEnergyItemEntity.getInstance()
                .cache(String.format("BaseData:ChargeStandardEnergy:%s:Item", chargeStandardConfigId))
                .where("configId", chargeStandardConfigId)
                .page(1, 100)
                .order("id")
                .selectList();
        if (itemList == null || itemList.isEmpty()) {
            LogsUtil.warn(TAG, "计算电费错误，计费配置错误");
            return new BigDecimal(0);
        }

        BigDecimal electricityFeeAmount;
        ChargeStandardEnergyItemEntity itemEntity = itemList.get(0);
        //电费 = 电量 * 价格  这里只是简略计算，只是针对单一时段计费而已
        electricityFeeAmount = totalElectricity.multiply(itemEntity.elecPrice);
        return electricityFeeAmount.setScale(6, RoundingMode.HALF_UP);
    }

    /**
     * 计算服务费 = 电量 * 价格 (临时使用)
     * TODO 分时段计算服务费（这里只是简单计算，针对单一时段计费而已），计费需要考虑分时段计费，没那么简单，这里暂时使用固定公式，后面需要进行优化
     *
     * @return
     */
    public static BigDecimal billingServiceAmountTEMP(BigDecimal totalElectricity, long chargeStandardConfigId) {
        if (totalElectricity.compareTo(BigDecimal.ZERO) == 0) {
            LogsUtil.warn(TAG, "计算电费错误，无总电量");
            return new BigDecimal(0);
        }
        ...
        ChargeStandardEnergyItemEntity itemEntity = itemList.get(0);
        //电费 = 电量 * 价格  这里只是简略计算，只是针对单一时段计费而已
        amount = totalElectricity.multiply(itemEntity.servicePrice);
