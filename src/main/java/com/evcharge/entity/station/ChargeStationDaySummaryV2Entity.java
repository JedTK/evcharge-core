...
@Getter
@Setter
public class ChargeStationDaySummaryV2Entity extends BaseEntity implements Serializable {
    //region -- 实体类属性 --
    /**
     * id
     */
     ...
     * 获得一个实例
     */
    public static ChargeStationDaySummaryV2Entity getInstance() {
        return new ChargeStationDaySummaryV2Entity();
    }

    private final static String TAG = "充电桩日汇总v2";
...
//                    .find();
            ...
            data.put("recharge_refund_order_count", MapUtil.getInt(rechargeRefundData, "refund_order_count"));//充值退款订单数
...
//                    .find();
            ...
            if (total_socket > 0) {
                ...
                //region [端口收益金额] 创造的收益(元/日/插座): (充值 + 充电卡 - 充值退款 - 充电卡退款) /  运行中的充电端口数
                BigDecimal net_income = new BigDecimal(0); // socketRevenue
                ...
                // region 端口消费金额(元/日/插座): (充电消费金额 + 充电卡消费金额 - 充电消费调整金额 - 充电卡消费调整金额) /  运行中的充电端口数
        ...
        while (true) {
            ...
            for (Map<String, Object> nd : list) {
                ...
                if (useRocketMQ) {
                    ...
                    XRocketMQ.getGlobal().pushSync(ChargeStationXRocketMQConsumerV2.TOPIC, "DaySummaryTaskV2", rocketMQData);
                } else {
                    // 普通线程任务
