 ...
 * 充电桩月数据汇总
 */
public class ChargeStationMonthlySummaryEntity extends BaseEntity implements Serializable {
    //region -- 实体类属性 --
    /**
     * id
     */
     ...
     * 获得一个实例
     */
    public static ChargeStationMonthlySummaryEntity getInstance() {
        return new ChargeStationMonthlySummaryEntity();
    }
...
//                LogsUtil.info("充电桩月数据汇总", "CSId=%s %s - 当天 总充电时间 %s 毫秒", CSId, date, currentChargeTime);
            }
...
//                LogsUtil.info("充电桩月数据汇总", "CSId=%s %s - 前一天跨天 总充电时间 %s 毫秒", CSId, date, beforeChargeTime);
            }
...
//                LogsUtil.info("充电桩月数据汇总", "CSId=%s %s - 当天跨天 总充电时间 %s 毫秒", CSId, date, afterChargeTime);
            }
...
//                    .find();
            ...
            if (totalSocket > 0) {
                ...
                chargeStationSummaryData.put("chargeCountMonthUseRate", chargeCountUseRate);
                //endregion


                //region (%)时长使用率（APR）：当月充电时长「秒数」 / （不含私有桩）(充电桩A总端口运行时间 + 充电桩B总端口运行时间...)
                //此充电桩总端口当月运行时间 = 充电桩的运营时间 * 端口数
                ...
                chargeStationSummaryData.put("chargeTimeMonthUseRate", chargeTimeUseRate);
                //endregion
                ...
                Map<String, Object> dailySummaryData = ChargeStationDailySummaryEntity.getInstance()
            ...
            if (getInstance().where("CSId", CSId).where("date", date).exist()) {
                getInstance().where("CSId", CSId).where("date", date).update(data);
            } else {
                data.put("CSId", CSId);
                data.put("date", date);
            ...
            LogsUtil.error(e, "充电桩月数据汇总", "汇总数据发生错误，CSId=%s date=%s", CSId, date);
        }
     ...
     * 内嵌自己来重复修复（辅助使用）
     */
    public SyncResult repairSysncData(long CSId, long date_timestamp) {
        long startTime = TimeUtil.toMonthBegin00(date_timestamp);

        String date = TimeUtil.toTimeString(startTime, "yyyy-MM");
        LogsUtil.info("充电桩月数据汇总 - 修复数据任务", "CSId = %s 开始修复 %s 的数据", CSId, date);

        long month = TimeUtil.getMonthBegin00();
        if (startTime <= month) {
            SyncResult r = syncData(CSId, startTime);
            if (r.code == 0) {
                startTime = TimeUtil.getAddMonthTimestamp(startTime, 1);
            }
            repairSysncData(CSId, startTime);
        }
        LogsUtil.info("充电桩月数据汇总 - 修复数据任务", "CSId = %s 修复结束！！！截止时间：%s", CSId, date);
...
//            LogsUtil.info("充电桩月数据汇总 - 修复数据任务", "CSId = %s - %s 新增任务从 %s 开始修复", CSId, name, date);
            ThreadUtil.getInstance().execute("", () -> {
                //从指定日期开始修复
                if (start_time > 0) {
                    repairSysncData(CSId, start_time);
                    return;
                }
                repairSysncData(CSId, online_time);
            });
        }
    }
}
