 ...
 * @date : 2023-8-14
 */
public class PlatformDailySummaryEntity extends BaseEntity implements Serializable {
    //region -- 实体类属性 --
    /**
     * id
     */
     ...
     * @return
     */
    public static PlatformDailySummaryEntity getInstance() {
        return new PlatformDailySummaryEntity();
    }
...
//                    LogsUtil.info("平台日汇总 - 计次充电的时长", "%s - 当天 总充电时间 %s 毫秒", date, currentChargeTime);
                }
...
//                    LogsUtil.info("平台日汇总 - 计次充电的时长", "%s - 前一天跨天 总充电时间 %s 毫秒", date, beforeChargeTime);
                }
...
//                    LogsUtil.info("平台日汇总 - 计次充电的时长", "%s - 当天跨天 总充电时间 %s 毫秒", date, afterChargeTime);
                }
...
//                    LogsUtil.info("平台日汇总 - 充电卡充电的时长", "%s - 当天 总充电时间 %s 毫秒", date, currentChargeTime);
                }
...
//                    LogsUtil.info("平台日汇总 - 充电卡充电的时长", "%s - 前一天跨天 总充电时间 %s 毫秒", date, beforeChargeTime);
                }
...
//                    LogsUtil.info("平台日汇总 - 充电卡充电的时长", "%s - 当天跨天 总充电时间 %s 毫秒", date, afterChargeTime);
                }
...
//                    .find();
            ...
            if (totalSocket > 0) {
                ...
                //region (元/日/插座)插座净收入：(充值 + 充电卡 - 充值退款 - 充电卡退款) /  全平台运行中的充电端口
...
//                    .selectForArray("uid");
            ...
            LogsUtil.error(e, "平台日数据汇总", "汇总数据发生错误，date=%s", date);
        }
    ...
    public SyncResult repairSyncData(long date_timestamp) {
        String date = TimeUtil.toTimeString(date_timestamp, "yyyy-MM-dd");
        LogsUtil.info("平台日数据汇总 - 修复数据任务", "%s 开始修复数据", date);

        long startTime = TimeUtil.toTimestamp00(date_timestamp);
        long today = TimeUtil.getTime00();
        if (startTime <= today) {
            SyncResult r = syncData(date_timestamp);
            if (r.code == 0) date_timestamp += ECacheTime.DAY;
            repairSyncData(date_timestamp);
        }
        LogsUtil.info("平台日数据汇总 - 修复数据任务", "%s 修复结束！！！", date);
