...
@Getter
@Setter
public class PlatformMonthSummaryV2Entity extends BaseEntity implements Serializable {
    //region -- 实体类属性 --
    /**
     * id
     */
     ...
     * 获得一个实例
     */
    public static PlatformMonthSummaryV2Entity getInstance() {
        return new PlatformMonthSummaryV2Entity();
    }

    private final static String TAG = "平台月汇v2";
    ...
    public SyncResult syncTaskJob(String organize_code, long date_timestamp) {
        ...
        try {
            ...
            data.put("total_power_consumption", sumCount.get("total_power_consumption"));//累计耗电量（度）
            ...
            //当前全平台用户余额
            BigDecimal total_user_balance;
            //应税收入（总充电消费金额），公式：计次充电消费+充电卡金额-充电卡退款金额
            BigDecimal taxable_income;
...
//                    .find();
            ...
            if (total_socket > 0) {
                ...
                BigDecimal charge_count_use_rate = BigDecimal.valueOf(total_use_count).divide(BigDecimal.valueOf(total_socket), 6, RoundingMode.HALF_UP);
                ...
                if (total_socket_run_time > 0) {
                    charge_time_use_rate = BigDecimal.valueOf(total_charge_time).divide(BigDecimal.valueOf(total_socket_run_time), 6, RoundingMode.HALF_UP);
                }
...
//                    .selectForArray("uid");
    ...
    public void syncData(String organize_code, long date_timestamp, long end_time) {
        ...
        while (startTime <= month) {
            syncTaskJob(organize_code, startTime);
    ...
    public void startSyncTask(long start_time, long end_time, boolean useRocketMQ) {
        ...
        while (true) {
            ...
            List<Map<String, Object>> list = RBOrganizeEntity.getInstance()
                    ...
                    .where("code", "!=", "")
                    .page(page, limit) // 分页仅在RocketMQ模式下有效
                    .select();
            ...
            for (Map<String, Object> data : list) {
                ...
                if (useRocketMQ) {
                    ...
                    XRocketMQ.getGlobal().pushOneway(PlatformXRocketMQConsumerV2.TOPIC, "MonthSummaryTaskV2", rocketMQData);
                } else {
                    // 普通线程任务
