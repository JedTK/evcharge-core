...
@Getter
@Setter
public class PlatformSummaryV2Entity extends BaseEntity implements Serializable {
    //region -- 实体类属性 --
    /**
     * id
     */
     ...
     * 获得一个实例
     */
    public static PlatformSummaryV2Entity getInstance() {
        return new PlatformSummaryV2Entity();
    }

    private final static String TAG = "平台汇总v2";

    /**
     * 同步任务作业
     */
    public SyncResult syncTaskJob(String organize_code) {
        try {
            if (!StringUtil.hasLength(organize_code)) return new SyncResult(1, "");
            LogsUtil.info(TAG, "[%s] 正在统计数据...", organize_code);
            ...
            if (total_socket > 0) {
                ...
                if (chargeStationEntityList != null) {
                    for (Map<String, Object> nd : chargeStationEntityList) {
                        ...
                        total_socket_run_time += (TimeUtil.getTimestamp() - online_time) / 1000 * totalSocket_temp;
                    }
                }
                BigDecimal charge_time_use_rate = BigDecimal.valueOf(total_charge_time)
                ...
                Map<String, Object> daySummaryData = PlatformDaySummaryV2Entity.getInstance()
                ...
                Map<String, Object> totalNetIncomeData = PlatformDaySummaryV2Entity.getInstance()
...
//                    .selectForArray("uid");
            ...
            if (getInstance().where("organize_code", organize_code).exist()) {
                getInstance().where("organize_code", organize_code).update(data);
            } else {
        ...
        LogsUtil.info(TAG, "[%s] 统计数据 完成！", organize_code);
        ...
        while (true) {
            ...
            for (Map<String, Object> data : list) {
                ...
                if (useRocketMQ) {
                    ...
                    XRocketMQ.getGlobal().pushOneway(PlatformXRocketMQConsumerV2.TOPIC, "SummaryTaskV2", rocketMQData);
                } else {
                    // 普通线程任务
                    ThreadUtil.getInstance().execute(String.format("[%s] %s", organize_name, TAG), () -> syncTaskJob(organize_code));
                }
            }
        }
    }
