...
import com.evcharge.mqtt.IXMQTTClient;
import com.evcharge.mqtt.IXMQTTSubMessageListener;
import com.evcharge.mqtt.XMQTTSub;
import com.evcharge.mqtt.XMQTTUtils;
...
public class XMQTTClientV5 implements IXMQTTClient {
    ...
    public void connectPub(String brokerServer, String ClientId, String UserName, String Password, IAsyncListener iAsyncListener) {
        ...
        try {
            ...
            // 创建异步 MQTT 5 客户端
            mPubClient = MqttClient.builder()
                    .useMqttVersion5()                 // 使用 MQTT 5 协议
                    .identifier(ClientId)              // 客户端唯一标识（ClientId）
                    .serverHost(host)                  // 设置服务器地址
                    .serverPort(port)                  // 设置服务器端口
                    .automaticReconnect(               // 自动重连策略配置
                            MqttClientAutoReconnect.builder()
                                    .initialDelay(1, TimeUnit.SECONDS)   // 初始重连间隔：1秒
                                    .maxDelay(2, TimeUnit.MINUTES)       // 最大重连间隔：2分钟
                                    .build())
                    .addConnectedListener(ctx ->       // 连接成功监听器
                            LogsUtil.info(SysTAG, "[推送] - ✅ 已连接，心跳 20s，监听自动重连"))
                    .addDisconnectedListener(ctx ->    // 断开连接监听器
                            LogsUtil.warn(SysTAG, "[推送] - ⚠️ 已断开，尝试自动重连"))
            ...
            mPubClient.connectWith()
                    ...
                    .password(Password.getBytes())  // 设置密码
                    ...
                    .whenComplete((connAck, throwable) -> {
                        ...
                        if (throwable != null) {
                            // 这里可以做手动重连
                            LogsUtil.error(SysTAG, "[推送] - ⚠️ 连接失败：%s", throwable.getMessage());
                            if (iAsyncListener != null) {
                                iAsyncListener.onResult(1, "连接失败：" + throwable.getMessage());
                            }
                        } else {
                            LogsUtil.info(SysTAG, "[推送] - ✅ 连接成功");
        ...
        try {
            ...
            mSubClient = MqttClient.builder()
                    ...
                    .automaticReconnect(               // 自动重连策略配置
                            MqttClientAutoReconnect.builder()
                                    ...
                                    .build())
                    .addConnectedListener(ctx ->       // 连接成功监听器
                            LogsUtil.info(SysTAG, "[订阅] - ✅ 已连接，心跳 20s，监听自动重连"))
                    .addDisconnectedListener(ctx ->    // 断开连接监听器
                            LogsUtil.warn(SysTAG, "[订阅] - ⚠️ 已断开，尝试自动重连"))
                    .transportConfig()                 // 网络传输参数配置
                    .mqttConnectTimeout(10, TimeUnit.SECONDS)  // MQTT CONNECT 报文的超时时间
                    .socketConnectTimeout(5, TimeUnit.SECONDS) // TCP 连接超时时间
                    .applyTransportConfig()            // 应用网络传输配置
            ...
            mSubClient.connectWith()
                    ...
                    .whenComplete((connAck, throwable) -> {
                        ...
                        if (throwable != null) {
                            // 这里可以做手动重连
                            LogsUtil.error(SysTAG, "[订阅] - ⚠️ 连接失败：%s", throwable.getMessage());
                            if (iAsyncListener != null) {
                                iAsyncListener.onResult(1, "连接失败：" + throwable.getMessage());
                            }
                        } else {
                            LogsUtil.info(SysTAG, "[订阅] - ✅ 连接成功");
                            if (iAsyncListener != null) iAsyncListener.onResult(0, "连接成功");
                        }
                    });
        } catch (Exception e) {
    ...
    public void publish(@NonNull String topic, String content, int qos, boolean retained, IAsyncListener iAsyncListener) {
        try {
            ...
            if (StringUtil.isEmpty(topic)) {
                LogsUtil.warn(SysTAG, "[推送] - 主题为空，取消发送");
                if (iAsyncListener != null) iAsyncListener.onResult(2, "推送主题为空，取消发送");
                return;
            }

            LogsUtil.info(SysTAG, "[推送] - 主题：%s Qos：%s 保留：%s 消息：%s", topic, qos, retained, content);

            if (mPubClient == null) {
                LogsUtil.error(SysTAG, "[推送] - 客户端未初始化");
            ...
            if (!mPubClient.getState().isConnected()) {
                LogsUtil.warn(SysTAG, "[推送] - 客户端未连接，消息未发送");
            ...
            mPubClient.publishWith()
                    ...
                    .whenComplete((ack, throwable) -> {
                        if (throwable != null) {
                            LogsUtil.warn(SysTAG, "[推送] - 主题：%s Qos：%s 保留：%s 消息：%s - 消息发送失败：%s", ack.getPublish().getTopic(), qos, retained, content, throwable.getMessage());
                            if (iAsyncListener != null)
            ...
            LogsUtil.error(e, SysTAG, "[推送] - 发送异常");
    ...
    public boolean subscribe(@NonNull String topic, int qos, IXMQTTSubMessageListener subMessageListener) {
        subscribe(topic, qos, subMessageListener, (resultCode, asyncResult) -> {
            if (resultCode != 0) LogsUtil.warn(SysTAG, asyncResult.toString());
        });
        return true;
    }
    ...
    public void subscribe(@NonNull String topic, int qos, IXMQTTSubMessageListener subMessageListener, IAsyncListener iAsyncListener) {
        try {
            ...
            LogsUtil.info(SysTAG, "[订阅] - 主题：%s QoS：%d", topic, qos);
            ...
            // 注册订阅监听器（onMessage）
            mSubClient.toAsync().subscribeWith()
                    .topicFilter(topic)
                    .qos(mqttQos)
                    .callback(publish -> {
    ...
    public boolean unsubscribe(@NonNull String topic) {
        try {
            ...
            if (StringUtil.isEmpty(topic)) {
                LogsUtil.warn(SysTAG, "[取消订阅] - 主题为空");
                return false;
            }
            ...
            if (mSubClient == null || !mSubClient.getState().isConnected()) {
                LogsUtil.warn(SysTAG, "[取消订阅] - 客户端未连接或未初始化");
                return false;
            }

            @NonNull String finalTopic = topic;
            mSubClient.unsubscribeWith()
                    .topicFilter(topic)
                    .send()
                    .whenComplete((ack, throwable) -> {
                        if (throwable != null) {
                            LogsUtil.error(SysTAG, "[取消订阅] 主题：%s 操作失败：%s", finalTopic, throwable.getMessage());
                        }
                    });
            return true;
        } catch (Exception e) {
            LogsUtil.error(e, SysTAG, "[取消订阅] 异常");
            return false;
        }
    }
    ...
    public boolean unsubscribe(@NonNull String[] topics) {
        if (topics.length == 0) {
            LogsUtil.warn(SysTAG, "[取消订阅] - 空的主题列表");
            return false;
        }
        ...
        try {
            List<MqttTopicFilter> filters = Arrays.stream(topics)
                    .map(t -> MqttTopicFilter.of(XMQTTUtils.formatTopic(replaceTopicEnvParams(t))))
                    .collect(Collectors.toList());

            mSubClient.unsubscribeWith()
                    .addTopicFilters(filters)
                    .send()
                    .whenComplete((ack, throwable) -> {
                        if (throwable != null) {
                            LogsUtil.error(SysTAG, "[批量取消订阅] 失败:%s", throwable.getMessage());
                        } else {
                            LogsUtil.info(SysTAG, "[批量取消订阅] 成功 - 共 %d 个主题", filters.size());
                        }
                    });
            return true;
        } catch (Exception e) {
            LogsUtil.error(e, SysTAG, "[批量取消订阅] 异常");
            return false;
        }
    }
    ...
    public void addSubListener(Class target) {
        ...
        for (Method m : methods) {
            ...
            subscribe(topic, xmqttSub.Qos(), (topic1, qos, message) -> {
                try {
                    if (xmqttSub.openLog()) {
                        LogsUtil.info(SysTAG, "收到订阅消息 - 主题：%s Qos:%s 消息", topic1, qos, message);
                    }
                    //判断是否是静态的
