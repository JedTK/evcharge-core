 ...
 */
public class SyncReChargeRefundOrderTask extends DBSyncTaskTrigger {
    /**
     * 获取实例（标准构造模式）
     */
    public static SyncReChargeRefundOrderTask getInstance() {
        return new SyncReChargeRefundOrderTask();
    }

    /**
     * 构造函数：初始化同步任务配置，包括主从表字段映射等
     */
    public SyncReChargeRefundOrderTask() {
        this.TAG = "充值订单同步触发器";
        task = new DBSyncTask();

        task.mainDBName = "";
        task.mainTableName = RechargeRefundOrderEntity.getInstance().theTableName();
